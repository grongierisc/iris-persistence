...
IRIS_DBAPI = iris.dbapi

IRIS_REMOTE_PORT = int(os.environ.get("IRIS_PORT", "1972"))
IRIS_REMOTE_HOST = os.environ.get("IRIS_HOST", "localhost")
IRIS_REMOTE_NAMESPACE = os.environ.get("IRIS_NAMESPACE", "USER")
IRIS_REMOTE_USERNAME = os.environ.get("IRISUSERNAME")
IRIS_REMOTE_PASSWORD = "SYS"
...
def _remote_connection():
    ...
    port = IRIS_REMOTE_PORT
    namespace = IRIS_REMOTE_NAMESPACE
    username = IRIS_REMOTE_USERNAME
    password = IRIS_REMOTE_PASSWORD
    missing = [
        name
        for name, value in (
...
def _load_benchmark_classes_on_current_runtime() -> None:
    ...
    for source in OBJECTSCRIPT_SOURCES:
        ...
        status = stream.Write(source.read_text())
        if not status:
            raise RuntimeError(f"Failed to write benchmark source to stream: {source}")
        stream.Rewind()
        status = system_obj.LoadStream(stream, "cuk")
        if not status:
...
def _run_dbapi_benchmark(mode: str, rows: int, iteration: int) -> BenchmarkRun:
    _ensure_benchmark_classes_loaded()
    _reset_extent()

    table_name = _resolve_sql_table_name(SimpleBenchmarkRecord)
    conn = None
    if mode == "embedded_dbapi":
        conn = IRIS_DBAPI.connect()
    else:
        conn = IRIS_DBAPI.connect(
            hostname=IRIS_REMOTE_HOST,
            port=IRIS_REMOTE_PORT,
            namespace=IRIS_REMOTE_NAMESPACE,
            username=IRIS_REMOTE_USERNAME,
            password=IRIS_REMOTE_PASSWORD,
    ...
    try:
        ...
        # Write phase: INSERT rows one by one
        start = time.perf_counter()
        for index in range(rows):
            cursor.execute(
                f"INSERT INTO {table_name} (Name, Category, Quantity, Price, Active)"
                " VALUES (?, ?, ?, ?, ?)",
                (
                    f"name-{index}",
                    f"group-{index % 10}",
        ...
        # Collect inserted IDs for the read phase (not counted in write time)
        cursor.execute(f"SELECT ID FROM {table_name} ORDER BY ID")
        id_rows = cursor.fetchall()
        if len(id_rows) != rows:
            raise RuntimeError(
                f"{mode} dbapi benchmark: expected {rows} IDs after insert, got {len(id_rows)}"
            )
        ids = [row[0] for row in id_rows]

        # Read phase: one SELECT per row
        start = time.perf_counter()
        for row_id in ids:
            cursor.execute(
                f"SELECT Name, Category, Quantity, Price, Active"
            ...
            if row is not None:
                if len(row) != 5:
                    raise RuntimeError(
                        f"{mode} dbapi benchmark read expected 5 columns, got {len(row)}"
                    )
                _consume_loaded_record(SimpleBenchmarkRecord(
                    Name=row[0],
                    Category=row[1],
                    Quantity=row[2],
                    Price=row[3],
            ...
            if row is None:
                raise RuntimeError(f"{mode} dbapi benchmark read failed for id={row_id}")
        read_seconds = time.perf_counter() - start

        # Fetch-all phase: single SELECT, fetch all rows at once
        start = time.perf_counter()
        cursor.execute(
            f"SELECT Name, Category, Quantity, Price, Active FROM {table_name}"
        )
        fetched = cursor.fetchall()
        for row in fetched:
            if len(row) != 5:
                raise RuntimeError(
                    f"{mode} dbapi benchmark fetch-all expected 5 columns, got {len(row)}"
                )
            _consume_loaded_record(SimpleBenchmarkRecord(
                Name=row[0],
                Category=row[1],
        ...
        if len(fetched) != rows:
            raise RuntimeError(
                f"{mode} dbapi benchmark expected {rows} rows from fetch-all, got {len(fetched)}"
            )
        fetch_all_seconds = time.perf_counter() - start
